...
const getShadows = () =>
  [
    ...
    ...new Array(18).fill('0 25px 50px -12px rgb(0 0 0 / 0.25)'),
  ] as any;
...
// Get form components configuration
const getFormComponents = (mode: PaletteMode) => ({
  MuiTextField: {
    defaultProps: {
      size: 'small' as const,
    },
...
// Get dialog components configuration
...
// Get table components configuration
const getTableComponents = (mode: PaletteMode) => ({
  MuiTableHead: {
    styleOverrides: {
      root: {
        '& .MuiTableCell-head': {
...
const getComponents = (mode: PaletteMode) => ({
  ...
  ...getFormComponents(mode),
  ...getDialogComponents(),
  ...getTableComponents(mode),
  MuiChip: {
    styleOverrides: {
      root: {
        fontWeight: 500,
